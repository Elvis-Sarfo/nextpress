...
import { Grid3X3, ImagePlus, List, Search, X } from 'lucide-react';
      ...
      <div className="relative flex h-[min(92vh,68rem)] w-full max-w-7xl flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl">
        <div className="border-b border-slate-200 px-5 py-3">
          <div className="flex flex-wrap items-start justify-between gap-2 sm:gap-4">
            <div className="flex gap-1 min-w-[180px] flex-shrink-0">
              {/* <div className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium uppercase tracking-[0.12em] text-slate-600">
                <ImagePlus className="h-3 w-3" />
                Media Library
              </div> */}
              <div>
                <h2 className="text-lg font-semibold tracking-tight text-slate-950">Choose from your library</h2>
                <p className="text-xs text-slate-500">Search, filter, upload, and select a file.</p>
              </div>
            </div>
            ...
            <select
              ...
              className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-xs outline-none transition-colors focus:border-slate-300 min-w-[120px] w-full sm:w-auto"
            >
            ...
            <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto justify-end mt-2 sm:mt-0">
              <Button
          ...
          {/* <div className="mt-3 flex flex-col gap-2 md:flex-row">
            ...
            <select
              ...
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value)}
              className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-xs outline-none transition-colors focus:border-slate-300"
            >
              <option value="all">All types</option>
              <option value="image">Images</option>
              <option value="video">Videos</option>
